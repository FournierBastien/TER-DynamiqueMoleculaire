...
!**************************************************************************
 ...
 do i=ifirst,ilast
  write(510,'(f7.3,f20.17,i2)')real(x(i)),real(v(i)),name(i)
 enddo
 write(510,'(/)')
...
sumRandom=.0
do i=ifirst,20
    write(*,'("rand()=",f7.5)')rand()
    sumRandom=sumRandom + rand()
 enddo
write(*,'("sumRandt=",f7.5)')sumRandom
write(*,'("rand()=",f7.5)')rand()
write(*,'("rand()=",f7.5)')rand()
write(*,'("rand()=",f7.5)')rand()
 vmoy=sum(v(ifirst:ilast))
 write(*,'(" vmoyen1 =",f10.3)')vmoy
 vmoy=vmoy/float(n)
 write(*,'(" vmoyen2 =",f7.3)')vmoy
 !write(*,'(" v =",f7.3)')v
 write(*,'(" v[1]=",f20.17," - vmoy=",f20.17," avant =",f20.17)')v(2),vmoy,v(2)-vmoy
 do i=ifirst,ilast
    write(*,'(" v[",i2,"] avant=",f20.17)')i,v(i)
    v(i) = v(i) - vmoy
    write(*,'(" v[",i2,"] apres=",f20.17)')i,v(i)
 enddo
 !v(ifirst:ilast)=v(ifirst:ilast)-vmoy
 !write(*,'(" v1 =",f20.17)')v
 vmoy=sum(v(ifirst:ilast))
 !write(*,'(" v =",f7.3)')v
 write(*,'(" vmoyen =",f10.7)')sum(v(ifirst:ilast))
